...
impl IconParser {
    ...
    pub fn load_icon(filepath: &Path, dark_mode: bool) -> Result<Pixbuf> {
        ...
        let pixbuf = Pixbuf::from_file_at_scale(filepath, 48, 48, true).map_err(|e| e.into());

        pixbuf.and_then(|pixbuf| pixbuf.add_alpha(true, alpha.0, alpha.1, alpha.2).map_err(|e| e.into()))
    }
}
